...
const API_REGISTRO = window.QD_CONFIG.API_BASE + "/auth/registro.php";
const API_CHECK_EMAIL = window.QD_CONFIG.API_BASE + "/auth/check-email.php";
  ...
  return (
    <div className="reg-modal-overlay">
      <div className="reg-modal-box">
        <div
          className={`reg-modal-icono-wrap ${esExito ? "reg-modal--exito" : "reg-modal--error"}`}
        >
...
function RegistroPage() {
    ...
    correoTimer.current = setTimeout(async () => {
      try {
        const res = await fetch(
          API_CHECK_EMAIL + "?correo=" + encodeURIComponent(valor),
        );
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!data.ok) {
        setModal({
          tipo: "error",
          mensaje: data.mensaje || "Error al crear la cuenta.",
        });
        setLoading(false);
        return;
      }
      setModal({
        tipo: "exito",
        nombre: data.usuario?.nombre || `${nombre} ${apellido}`,
      });
    } catch {
      setModal({
        tipo: "error",
  ...
  const correoStatusIcon = () =>
    ({ checking: "⏳", available: "✅", taken: "❌" })[correoStatus] || null;
  const correoMsgClase = () =>
    ({ available: "success", taken: "error", checking: "info" })[
      correoStatus
    ] || "info";
    ...
    <div className="reg-page-wrapper">
      ...
      <div className="reg-card">
        ...
        <div className="reg-panel-brand">
          ...
          <p className="reg-brand-message">
            "Únete a nuestra comunidad de huéspedes"
          </p>
          <p className="reg-brand-sub">
            Crea tu cuenta y disfruta de
            <br />
            reservaciones rápidas y exclusivas.
          </p>
          <ul className="reg-benefits">
            <li>
              <span className="reg-benefit-icon">✨</span> Reservaciones
              prioritarias
            </li>
            <li>
              <span className="reg-benefit-icon">🏷️</span> Tarifas especiales
              para miembros
            </li>
            <li>
              <span className="reg-benefit-icon">📋</span> Historial de
              estancias
            </li>
            <li>
              <span className="reg-benefit-icon">🔔</span> Notificaciones y
              ofertas
            </li>
          </ul>
        </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="reg-row-2col">
              <div className="reg-field-group">
                <label className="reg-label" htmlFor="reg-nombre">
                  Nombre
                </label>
                ...
                {errors.nombre && (
                  <p className="reg-field-msg error">
                    <IconAlert /> {errors.nombre}
                  </p>
                )}
              </div>
              <div className="reg-field-group">
                <label className="reg-label" htmlFor="reg-apellido">
                  Apellido
                </label>
                <div className="reg-input-wrap">
                  <input
            ...
            {/* Correo */}
            <div className="reg-field-group">
              <label className="reg-label" htmlFor="reg-correo">
                Correo Electrónico
              </label>
              <div className="reg-input-wrap">
                <input
              ...
              {(errors.correo || correoMsgTexto()) && (
                <p
                  className={`reg-field-msg ${errors.correo ? "error" : correoMsgClase()}`}
                >
                  {errors.correo ? (
                    <>
            ...
            <div className="reg-field-group">
              <label className="reg-label" htmlFor="reg-confirmar">
                Confirmar Contraseña
              </label>
              <div className="reg-input-wrap">
                <input
              ...
              {errors.confirmar && (
                <p className="reg-field-msg error">
                  <IconAlert /> {errors.confirmar}
                </p>
              )}
              {!errors.confirmar && confirmar && confirmar === password && (
                <p className="reg-field-msg success">
            ...
            {/* Captcha */}
            <div
              className={`reg-captcha-box${errors.captcha ? " captcha-error" : ""}`}
              onClick={() => {
                setCaptchaOk((p) => !p);
            ...
            >
              ...
              />
              <label htmlFor="reg-captcha" className="reg-captcha-text">
                No soy un robot
              </label>
...
const registroRoot = ReactDOM.createRoot(
  document.getElementById("root-registro"),
);
