(function () {
  ...
  //  SISTEMA DE TOASTS — window.QDToas
  ...
  window.QDToast = (function () {
    ...
    var cfg = {
      success: {
        icon: "✅",
        bg: "linear-gradient(135deg,#1a5c2e,#27ae60)",
        bo: "rgba(39,174,96,0.4)",
      },
      error: {
        icon: "❌",
        bg: "linear-gradient(135deg,#2a1206,#6b3a1f)",
        bo: "rgba(196,154,108,0.4)",
      },
      warning: {
        icon: "⚠️",
        bg: "linear-gradient(135deg,#7c4a00,#d97706)",
        bo: "rgba(217,119,6,0.4)",
      },
      info: {
        icon: "ℹ️",
        bg: "linear-gradient(135deg,#1e3a6e,#2563eb)",
        bo: "rgba(37,99,235,0.4)",
      },
    };
    ...
    function getCont() {
      ...
      cont.style.cssText =
        "position:fixed;bottom:24px;right:24px;z-index:99999;display:flex;flex-direction:column-reverse;gap:10px;max-width:360px;width:calc(100% - 48px);pointer-events:none;";
    ...
    return {
      mostrar: show,
      exito: function (m, d) {
        show(m, "success", d || 4000);
      },
      error: function (m, d) {
        show(m, "error", d || 5000);
      },
      aviso: function (m, d) {
        show(m, "warning", d || 4500);
      },
      info: function (m, d) {
        show(m, "info", d || 3500);
      },
  ...
  //  GESTOR DE SESION — window.QDSession
  window.QDSession = {
    ...
    guardar: function (u) {
      ...
      try {
        localStorage.setItem("qdTabEvt", "login|" + Date.now());
      } catch (e) {}
    ...
    cerrar: function (silencioso) {
      ...
      try {
        localStorage.setItem("qdTabEvt", "logout|" + Date.now());
      } catch (e) {}
      try {
        fetch("/Hotel-quinta-dalam/api/auth/logout.php", {
          method: "POST",
          credentials: "include",
        }).catch(function () {});
      } catch (e) {}
    ...
    resetActividad: function () {
      ...
      timerAviso = setTimeout(function () {
        QDSession.mostrarAviso();
      }, AVISO_MS);
      timerExpiracion = setTimeout(function () {
        QDSession.cerrar(false);
      }, TIMEOUT_MS);
    },

    // HEARTBEAT
    //   Envia la cookie PHPSESSID al servidor para validar
    //   la sesion PHP. PHP verifica el fingerprint (User-Agent).
    //   Si la sesión es valida y el fingerprint coincide,
    //   devuelve los datos del usuario.
    //   EXTRA: si una nueva pestaña no tiene sessionStorage
    //   pero sí tiene la cookie PHP valida, reconstituye
    //   la sesion automaticamente (soluciona tab sync).
    validarConServidor: function () {
      return fetch(API_VALIDATE, {
        ...
        .then(function (data) {
          if (!data.ok) {
            // Sesion PHP inválida o fingerprint no coincide
