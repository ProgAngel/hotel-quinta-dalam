...
function ModuloReservaciones() {
  // ── Sesión
  const sesion =
    (window.QDSession && window.QDSession.obtener()) ||
    (() => {
      try {
    ...
    fetch(API_HABITACIONES + "?estado=disponible")
      ...
      .then((data) => {
        if (data.ok) setHabitaciones(data.habitaciones);
          ...
          window.QDToast &&
            window.QDToast.error("Error al cargar habitaciones.");
      })
      .catch(() => {
        window.QDToast &&
          window.QDToast.error("No se pudo conectar con el servidor.");
      })
  ...
  const habsParaHuespedes = habitaciones.filter(
    (h) => h.capacidad >= huespedesTotales,
  );
  const subtotal =
    habSeleccionada && noches > 0 ? habSeleccionada.precio_noche * noches : 0;
  ...
  async function procesarReserva(e) {
    ...
    try {
      ...
      if (!dataReserva.ok) {
        window.QDToast &&
          window.QDToast.error(
            dataReserva.mensaje || "No se pudo crear la reservación.",
          );
        return;
      }
      ...
      const metodoPago =
        document.getElementById("modal-metodo-pago")?.value || "recepcion";

      if (metodoPago === "tarjeta") {
        // Pago en línea con Mercado Pago
        window.QDToast &&
          window.QDToast.info("Conectando con Mercado Pago...", 3000);
        ...
        if (!dataPago.ok) {
          window.QDToast &&
            window.QDToast.error(
              dataPago.mensaje || "Error al conectar con Mercado Pago.",
            );
          return;
        }
  ...
  return (
    <div className="reserva-layout">
      ...
      <div className="reserva-col" id="paso-1" ref={refPaso1}>
        ...
        <div className="reserva-huespedes-fila">
          <div className="reserva-huesped-grupo">
            ...
            <div className="reserva-contador">
              <button
                ...
                onClick={() => setAdultos((p) => Math.max(1, p - 1))}
              >
                -
              </button>
              <span>{adultos}</span>
              <button
                className="reserva-contador__btn"
                onClick={() => setAdultos((p) => Math.min(8, p + 1))}
              >
                +
              </button>
            </div>
          ...
          <div className="reserva-huesped-grupo">
            ...
            <div className="reserva-contador">
              <button
                className="reserva-contador__btn"
                onClick={() => setNinos((p) => Math.max(0, p - 1))}
              >
                -
              </button>
              <span>{ninos}</span>
              <button
                className="reserva-contador__btn"
                onClick={() => setNinos((p) => Math.min(4, p + 1))}
              >
                +
              </button>
            </div>
        ...
        <div
          className={`reserva-estado-box reserva-estado-box--${estadoValidacion}`}
        >
          ...
          {noches > 0 && (
            <p className="reserva-noches">🌙 Total de noches: {noches}</p>
          )}
        </div>
      </div>
      ...
      <div
        className={`reserva-col ${noches === 0 ? "reserva-col--disabled" : ""}`}
        id="paso-2"
        ref={refPaso2}
      >
        ...
        {noches === 0 ? (
          ...
          <p style={{ fontFamily: "'Lato',sans-serif", color: "#aaa" }}>
          ...
          >
            ...
            <p style={{ color: "#888" }}>
              No hay habitaciones disponibles para {huespedesTotales} huéspedes
              en estas fechas.
            </p>
            ...
            {habsParaHuespedes.map((hab) => (
              ...
              >
                <img
                  src={
                    hab.imagen_url ||
                    `./img/habitaciones/habitacion${hab.numero}.jpg`
                  }
                  alt={hab.nombre}
                  loading="lazy"
                  onError={(e) => {
                    e.target.src =
                      "https://via.placeholder.com/150/e0d5c1/8c5a35?text=Sin+Foto";
                  }}
                />
                <h4>{hab.nombre}</h4>
                <p
                ...
                <p className="hab-mini-precio">
                  ${fmt(hab.precio_noche)}/noche
                </p>
      ...
      <div
        className={`reserva-col ${!habSeleccionada ? "reserva-col--disabled" : ""}`}
        id="paso-3"
        ref={refPaso3}
      >
        ...
        {habSeleccionada && noches > 0 ? (
          <div className="resumen-cuerpo">
            {/* Resumen */}
            <div>
              <h4 className="resumen-nombre">
                Habitación {habSeleccionada.nombre}
              </h4>
              <p
                style={{
                  fontFamily: "'Lato',sans-serif",
              ...
              <p className="resumen-detalle">
                <strong>Llegada:</strong> {checkIn}
              </p>
              <p className="resumen-detalle">
                <strong>Salida:</strong> {checkOut}
              </p>
              <p className="resumen-detalle">
                <strong>Estancia:</strong> {noches} noche
                {noches !== 1 ? "s" : ""}
              </p>
              <p className="resumen-detalle">
                <strong>Huéspedes:</strong> {adultos} Adulto
                {adultos !== 1 ? "s" : ""}, {ninos} Niño{ninos !== 1 ? "s" : ""}
              </p>
              <p className="resumen-detalle">
                <strong>Reservado por:</strong>{" "}
                <span style={{ color: "#8c5a35", fontWeight: 700 }}>
        ...
        <div className="modal-reserva-overlay">
          <div className="modal-reserva-box">
            ...
            <select
              ...
              id="modal-metodo-pago"
              defaultValue="recepcion"
            >
              <option value="recepcion">
                🏨 Pagar en Recepción (Efectivo / SPEI)
              </option>
        ...
        <div className="modal-cancelar-overlay">
          <div className="modal-cancelar-box">
            ...
            <p className="modal-cancelar-texto">
              Se perderán los datos ingresados y la habitación seleccionada
              quedará disponible.
            </p>
            <div className="modal-cancelar-acciones">
              <button
                className="btn-volver"
                onClick={() => setMostrarCancel(false)}
              >
                No, volver
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  window.QDToast &&
                    window.QDToast.info(
                      "Reservación cancelada. Los datos fueron limpiados.",
                    );
                }}
              >
                Sí, cancelar
