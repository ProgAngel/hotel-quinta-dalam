...
const API_PERFIL = window.QD_CONFIG.API_BASE + "/usuarios/perfil.php";
const API_RESERVACIONES =
  window.QD_CONFIG.API_BASE + "/reservaciones/listar.php";
...
const totalFmt = (t) =>
  "$" +
  Number(t).toLocaleString("es-MX", { minimumFractionDigits: 2 }) +
  " MXN";
const fmtSimple = (t) =>
  "$" + Number(t).toLocaleString("es-MX", { minimumFractionDigits: 2 });
const colBadge = (e) =>
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            className={`perfil-badge ${colBadge(r.estado)}`}
        ...
        >
          <p>
            <strong style={{ color: "#2d1a0e" }}>Tipo de habitación:</strong>{" "}
            {r.habitacion_tipo}
          </p>
          <p>
            <strong style={{ color: "#2d1a0e" }}>Precio por noche:</strong>{" "}
            {fmtSimple(r.precio_noche)}
          </p>
          {r.notas && (
            <p>
              <strong style={{ color: "#2d1a0e" }}>Notas:</strong> {r.notas}
            </p>
          )}
          <p>
            <strong style={{ color: "#2d1a0e" }}>Reservación creada:</strong>{" "}
        ...
        {r.estado === "pendiente" && (
          ...
          >
            ⏳ Pago pendiente. Tienes 15 minutos para completar el pago antes de
            que se libere la habitación.
          </div>
        )}
        ...
        >
          {r.estado === "pendiente" && (
            <a
              href="reservaciones.html#paso-1"
              style={{
                padding: "10px 18px",
                borderRadius: "8px",
                border: "none",
  ...
  return (
    <div className="perfil-reserv-item">
      ...
      <div className="perfil-reserv-dates">
        <span>
          📅 Entrada: <strong>{formatFecha(reserv.fecha_entrada)}</strong>
        </span>
        <span>
          📅 Salida: <strong>{formatFecha(reserv.fecha_salida)}</strong>
        </span>
        <span>
          🌙 {reserv.noches} {reserv.noches === 1 ? "noche" : "noches"}
        </span>
      </div>
      <div className="perfil-reserv-footer">
        ...
        <button
          className="perfil-btn-link"
          onClick={() => onVerDetalle(reserv)}
        >
          👁️ Ver detalle
        </button>
      </div>
  ...
  async function handleGuardarPerfil(e) {
    ...
    try {
      ...
      if (!data.ok) {
        setAlertEdit({
          tipo: "error",
          msg: data.mensaje || "No se pudo actualizar.",
        });
        return;
      }

      if (window.QDSession) {
        window.QDSession.guardar({
        ...
        sessionStorage.setItem(
          "qdSession",
          JSON.stringify({ ...raw, nombre: nombre.trim() }),
        );
      }
    ...
    try {
      ...
      if (!data.ok) {
        if (data.errores?.contrasena_actual)
          ...
          setAlertPwd({
            tipo: "error",
            msg: data.mensaje || "No se pudo cambiar la contraseña.",
          });
        return;
      }
      setAlertPwd({
  ...
  return (
    <div className="perfil-page">
      ...
      <div className="perfil-hero">
        <div className="perfil-avatar">{loadingPerfil ? "..." : iniciales}</div>
                    ...
                    <div>
                      ...
                      <p className="perfil-info-value">
                        {perfilData?.created_at
                          ? new Date(perfilData.created_at).toLocaleDateString(
                              "es-MX",
                              {
                                year: "numeric",
                                month: "long",
                    ...
                    <div className="perfil-input-wrap">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setErrEdit((p) => ({ ...p, nombre: "" }));
                        }}
                      />
                      <span className="perfil-input-icon">👤</span>
                    </div>
                    {errEdit.nombre && (
                      <p className="perfil-field-error">
                        <IconAlert /> {errEdit.nombre}
                      </p>
                    )}
                  </div>
                  <div className="perfil-field-group">
            ...
            {loadingReserv ? (
              ...
              <>
                ...
                <p className="perfil-reserv-pager">
                  Mostrando {reservaciones.length} reservación
                  {reservaciones.length !== 1 ? "es" : ""}
                </p>
              </>
            )}
          </div>
